...
TFT::TFT(int8_t cs, int8_t dc, int8_t mosi,
...
TFT::TFT(int8_t cs, int8_t dc, int8_t rst) : Adafruit_GFX(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT) {
  _cs   = cs;
  _dc   = dc;
  _rst  = rst;
  hwSPI = true;
...
void TFT::writecommand(uint8_t c) {
  *dcport &=  ~dcpinmask;
...
void TFT::commandList(uint8_t *addr) {
...
void TFT::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1,
 uint16_t y1) {
...
/*
 ...
 uint16_t TFT::readcommand16(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 ...
 uint32_t TFT::readcommand32(uint8_t c) {
 digitalWrite(_dc, LOW);
 if (_cs)
 digitalWrite(_cs, LOW);
 spiwrite(c);
